...
- **Native `pnpm dev` collides with the docker stack's api/web containers.** When `docker compose up` is running, ports 3001 (api) and 5173 (web) are already bound. To run native dev against the same postgres + gotrue, first `docker stop prayer-api prayer-web` (keep postgres + gotrue running). The two containers serve images built from `main`, not the local source — so they're useless for testing branch changes anyway.
