...
import { MobileMemberProfilePage } from './MobileMemberProfilePage';
...
vi.mock('./MobileProfilePage', () => ({
  MobileProfilePage: () => <div data-testid="mobile-profile-page-mock">settings-mock</div>,
}));
vi.mock('./MobileSecurityPage', () => ({
  MobileSecurityPage: () => <div data-testid="mobile-security-page-mock">security-mock</div>,
}));
...
function renderAt(path: string): void {
  render(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        <Route path="/u/:userId" element={<MobileMemberProfilePage />} />
      </Routes>
    </MemoryRouter>,
  );
}

describe('MobileMemberProfilePage', () => {
  ...
  it('?tab=settings selects Settings tab', async () => {
    ...
    expect(screen.getByTestId('mobile-profile-page-mock')).toBeInTheDocument();
  });
});
