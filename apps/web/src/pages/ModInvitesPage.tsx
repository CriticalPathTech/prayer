...
export function ModInvitesPage(): JSX.Element {
  const { me } = useAuth();
  ...
  return (
    <div className="mx-auto max-w-feed">
      ...
      <section className="mb-8 rounded-md border border-[var(--border-soft)] bg-[var(--bg-raised)] p-4">
        <h2 className="mb-3 font-serif text-lg font-medium">Grant a new code</h2>
        <Field label="Member">
          <input
            aria-label="Member"
            value={m.query}
        ...
        {m.results.length > 0 ? (
          <ul className="mb-3 rounded-md border border-[var(--border-soft)] bg-[var(--bg-page)]">
            {m.results.map((u) => (
              <li key={u.id}>
                <button
                  type="button"
                  onClick={() => void m.selectUser(u)}
                  className="w-full p-2 text-left text-sm hover:bg-[var(--bg-raised)]"
                >
        ...
        {m.toast ? <p className="mt-3 text-sm text-sage-700">{m.toast}</p> : null}
        {m.error ? <p className="mt-3 text-sm text-ember-600">{m.error}</p> : null}
      </section>
      ...
      {m.selected ? (
        <section className="rounded-md border border-[var(--border-soft)] bg-[var(--bg-raised)] p-4">
          ...
          {m.codes.length === 0 ? (
            ...
            <ul>
              {m.codes.map((c) => (
                <li key={c.id} className="mb-3 flex items-center justify-between gap-3">
