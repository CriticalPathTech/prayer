...
- **`MobilePostCard` ≠ `PostCard`.** Two card components with different chrome: desktop uses solid `PrayButton`; mobile uses text-only "I Will Pray · N" + divider. Mobile views (`views/mobile/*`) must import `MobilePostCard` from `./MobilePostCard`, never `components/PostCard`. Both files exist and look similar — easy to grab the wrong one.
