...
export interface CreateTestAppOptions {
  host?: string; // default 'testchurch.prays.online'
}

export interface TestApp {
  app: import('express').Express;
  /** Supertest agent with Host header pre-set (default testchurch.prays.online). */
  agent: ReturnType<typeof supertest.agent>;
  db: Kysely<Database>;
  storage: InMemoryStorage;
  /** The org ID for the default host ('testchurch'). Use this when inserting users
...
export async function createTestApp(opts: CreateTestAppOptions = {}): Promise<TestApp> {
  const host = opts.host ?? 'testchurch.prays.online';
  ...
  // "testchurch" org (seeded in global-setup).
  ...
  const hostSlug = host.split('.')[0]!; // 'testchurch.prays.online' → 'testchurch'
