...
describe('cross-org isolation', () => {
  ...
  beforeAll(async () => {
    ...
    // orgA is the testchurch org seeded by global-setup — its slug matches the
    // default test Host (testchurch.prays.online), so userA's requests via the
    // default agent resolve correctly through orgContext + requireAuth.
    const testchurch = await app.db
      .selectFrom('orgs')
  ...
  describe('feed', () => {
    ...
    it('GET /feed — userB only sees orgB posts', async () => {
      // userB is a member of orgB only; default Host (testchurch) would 403.
  ...
  describe('multi-org member', () => {
    ...
    it('GET /me returns member role at orgA host', async () => {
      const res = await request(app.app)
        .get('/me')
        .set('Host', 'testchurch.prays.online')
