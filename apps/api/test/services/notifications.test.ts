...
describe('listNotifications', () => {
  ...
  beforeAll(async () => {
    ...
    orgId = await insertOrg(db, { slug: 'hopechurch-svc-notifs-list' });
  });
  afterAll(async () => {
    await db.destroy();
  });
...
describe('markNotificationRead', () => {
  ...
  beforeAll(async () => {
    db = initDb(process.env.TEST_DATABASE_URL!);
    orgId = await insertOrg(db, { slug: 'hopechurch-svc-notifs-read' });
  });
  afterAll(async () => {
    await db.destroy();
  });
...
describe('markAllNotificationsRead', () => {
  ...
  beforeAll(async () => {
    db = initDb(process.env.TEST_DATABASE_URL!);
    orgId = await insertOrg(db, { slug: 'hopechurch-svc-notifs-read-all' });
  });
  afterAll(async () => {
    await db.destroy();
  });
