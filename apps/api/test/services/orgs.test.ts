...
describe('findOrgByHost', () => {
  ...
  it('matches a known subdomain', async () => {
    const orgId = await insertOrg(app.db, { slug: 'testchurch-find' });
    const result = await findOrgByHost(app.db, 'testchurch-find.prays.online');
    expect(result?.id).toBe(orgId);
    expect(result?.slug).toBe('testchurch-find');
  });
